# App Packager SEA task list

This document tracks the first real implementation slice for `service-lasso-app-packager-sea`.
...
- adds a bounded Node SEA launcher wrapper around the canonical Node host payload
...
3. Add a real Node SEA launcher wrapper
   status: done

4. Add direct tests for config resolution, host routes, and artifact verification
   status: done

5. Add release artifacts that prove source, bootstrap-download, and bundled modes through the Node SEA wrapper
   status: done
...
**the canonical Node host can be wrapped with Node SEA, released with honest source/bootstrap/bundled outputs, and still surface Service Admin against real Echo Service**
