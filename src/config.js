...
export function resolveAppPackagerSeaConfig(options = {}) {
  const rootDir = options.repoRoot ?? repoRoot;
  const siblingRoot = options.siblingRoot ?? path.resolve(rootDir, "..");
  const hostPort =
    options.hostPort ??
    Number(process.env.SERVICE_LASSO_APP_PACKAGER_SEA_HOST_PORT ?? process.env.SERVICE_LASSO_APP_NODE_HOST_PORT ?? 19040);
  const runtimePort =
    options.runtimePort ??
    Number(process.env.SERVICE_LASSO_API_PORT ?? process.env.SERVICE_LASSO_APP_PACKAGER_SEA_API_PORT ?? 18084);
  const adminDistRoot =
  ...
  const workspaceBaseRoot =
    ...
    process.env.SERVICE_LASSO_APP_PACKAGER_SEA_WORKSPACE_BASE_ROOT ??
  ...
  const sourceServicesRoot =
    ...
    process.env.SERVICE_LASSO_APP_PACKAGER_SEA_SOURCE_SERVICES_ROOT ??
...
export async function validateAppPackagerSeaConfig(config) {
